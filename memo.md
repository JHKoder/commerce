...
### 쿼리 성능 이슈 2억 건 대량 insert (#58)
//임시) 초기 2억6천건 데이터 생성시 590,625시간 걸림 
// 해결책 )
// batch_size 조정
// jdbcTemplate batchInsert 사용 
// entity @id SEQUENCE 을 사용
// -> 1억건 데이터를 20분으로 단축 

### 3천만건 데이터에서 like 페이징  
//임시) 특정 검색문 like 조회시 7~ 10초 걸리는 특정 조건 페이징 1초로 시간 단축  
//해결책: 조인 최소화 , 쿼리 분리(inner select) 필요 데이터 획득 
// 캐싱 되면 0.003초 으로 단축
  
### JPA,SQL N + 1 문제의 해결 루트 
//임시) 자기참조 구조를 가진체로 특정 조건들을 구해오는데에서 sql 관점에서 n+1 문제가 발생함
//해결책 : path   ex(1/2, 1/2/3 , 1/2/4)
//     특정 문자를 조회하는 방식으로 n+1 문제를 해결
ㅡ
### 
