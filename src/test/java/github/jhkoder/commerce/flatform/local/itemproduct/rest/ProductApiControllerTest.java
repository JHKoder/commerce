...
public class ProductApiControllerTest extends RestDocControllerTests {
    ...
    void categoryPage() throws Exception {
        ...
        ResultActions actions = jsonGetWhen(api + "/category", request);
        ...
        actions.andExpect(status().isOk())
                .andDo(document(pathAdoc,
                        ...
                        responseFields(
                                ...
                                fieldWithPath("content[].clickCount").type(JsonFieldType.NUMBER).description("클릭 횟수"),
                                fieldWithPath("content[].deliveryPrice").type(JsonFieldType.NUMBER).description("기본 배송비")
                        )
    ...
    private Page<ProductsCategoryPageResponse> getCategoryPageProvider() {
        ...
        ProductsCategoryPageResponse response = new ProductsCategoryPageResponse(productId, imgLink, price, clickCount, deliveryPrice);
