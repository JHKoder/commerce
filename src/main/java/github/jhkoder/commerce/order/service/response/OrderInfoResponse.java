...
public record OrderInfoResponse(Long orderId,String name, String phone, String email,
    ...
    public static OrderInfoResponse of(User user, ItemProduct itemProduct, Order order) {
        return new OrderInfoResponse(order.getId(),user.getUserName(), user.getPhone(),user.getEmail(),
                itemProduct.getMainImage().getPath() ,itemProduct.getName(),order.getQuantity(),itemProduct.getOptionDetail(),itemProduct.getPrice(),itemProduct.getNormalPrice(),itemProduct.getDeliveryPrice()
        );
    }
}
